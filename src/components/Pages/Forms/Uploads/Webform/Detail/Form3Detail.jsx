...
export const Form3Detail = () => {
  ...
  const approvalCheck = checkApprovalProcess(getFormNumber(pathname), decryptedUser?.role_id, [ form3[0]?.is_dean && 9, form3[0]?.is_commex && 1, form3[0]?.is_asd && 10, form3[0]?.is_ad && 11, ].filter(Boolean), (owner?.role_id === 1 || owner?.role_id === 4), (owner?.role_id === 4))
  ...
  const handleViewRemarks = () => {
    ...
    const form3Remarks = event.form_remarks
      .filter(remark => 
        remark.form_type === 'form3' && 
        remark.form_id === form3[0]?.id
      )
  ...
  const onSubmitRevise = ({ remarks }) => {
    ...
    doReject({ 
      ...
      id: form3[0].id, 
      role_id: roleId,  
      remark: remarks // ✅ Change to unified 'remark' field
    });
    setShowRevise(false);
    navigate("/event/view");
  };
