...
export const Form3Detail = () => {
  ...
  const [ form3 ] = useState(initialData || null);
  ...
  const canAction = useMemo(() => {
    ...
    if (hasUserRoleApproved(form3)) return false;
  ...
  const { mutate: doReject, isLoading: rejectLoading } = useMutation({
    ...
    onSuccess: (res) => { toast(res?.message || "Sent for revision", { type: "success" }); reset({ remarks: "" }); },
  ...
  const onSubmitRevise = ({ remarks }) => {
    ...
    if (!key) return;
    doReject({ token: decryptedToken, id: form3[0].id, role_id: roleId,  [key]: remarks });
    setShowRevise(false);
  };
