...
export const Certificate = () => {
  ...
  useEffect(() => {
    ...
    let extractedDate = state?.implementDate || null;

    if ((!extractedNames.length || !extractedDate) && search.includes("data=")) {
      try {
        ...
        if (encoded) {
          ...
          extractedDate = decoded.implementDate || null;
        }
      } catch (err) {
    ...
    setImplementDate(extractedDate);
      ...
      {names.map((name, index) => (
        <div key={index} className="mb-10 break-after-page print:break-after-page">
          ...
          >
            ...
            {implementDate && (
              <div
                className="absolute text-black text-lg italic"
                style={{
                  top: "580px",
                  left: "51%",
                  transform: "translateX(-50%)",
              ...
              >
                <p>Implemented on: {new Date(implementDate).toLocaleDateString("en-US", {
                  year: "numeric",
                  month: "long",
                  day: "numeric",
                })}</p>
